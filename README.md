...
- **LaTeX 公式**：聊天、论文速览、来源摘要和领域报告共用 Markdown/KaTeX 渲染，支持 `$…$`、`$$…$$`、`\(…\)` 与 `\[…\]`，长公式可在面板内横向滚动。
...
- **可调布局**：拖拽侧栏边缘、阅读首页/话题输入区右侧，以及文档/对话、速览/PDF、来源/报告、设置分类栏之间的分隔条；提问区上边缘可调整输入高度，历史会话浮层左侧和底部可调整宽高。尺寸保存在当前浏览器，双击分隔条或聚焦后按 Enter 恢复默认，方向键微调、Shift 加速、Home/End 调到边界。窄屏保留面板切换与纵向布局。
